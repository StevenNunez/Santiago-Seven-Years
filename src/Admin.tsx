...
import { Clapperboard, Download, LoaderCircle, LockKeyhole, Save, Tv } from 'lucide-react';
import { db, errorMessage, supabase } from './supabase';
import { EVENT_ID } from './domain';
import type { Details, EventSettings, Profile } from './domain';
...
export default function Admin({ profile, event, details, refresh }: { profile: Profile | null; event: EventSettings; details: Details | null; refresh: () => Promise<void> }) {
  ...
  return <section className="section admin-section"><div className="section-heading"><span className="eyebrow">CENTRO DE MANDO</span><h2>Todo listo para el nivel 7</h2></div>
    ...
    <div className="card levels-card-admin"><span className="eyebrow">PARA PROYECTAR EN LA FIESTA</span><h3>Niveles anteriores</h3><p>La historia de Santiago del nivel 1 al 6 con música, y al final los mensajes de las familias confirmadas. Prepárala en el notebook que conectarás al televisor.</p><div className="button-row"><a className="button button-blue" href="#niveles"><Clapperboard size={18} />Abrir la presentación</a><a className="button button-outline" href="/?tv=1#niveles"><Tv size={18} />Modo TV (bucle)</a></div></div>
